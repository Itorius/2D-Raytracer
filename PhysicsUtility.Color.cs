...
namespace Raytracer
{
	public static partial class PhysicsUtility
	{
		...
		{
			if (wavelength < LEN_MIN || wavelength > LEN_MAX) throw new ArgumentOutOfRangeException(nameof(wavelength), $"Wavelength should be in range ({LEN_MIN}; {LEN_MAX})nm");

			wavelength -= LEN_MIN;
			int index = (int)Math.Floor(wavelength / LEN_STEP);
